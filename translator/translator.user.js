...
// @updateURL    https://github.com/zhao004/oil_monkey_script/blob/master/translator/translator.meta.js
...
const transdict = {
    ...
    '腾讯AI翻译': translate_tencentai, //'有道翻译':translate_youdao,
...
const startup = {
    ...
    '腾讯翻译': translate_tencent_startup, '彩云小译': translate_caiyun_startup, 'Papago翻译': translate_papago_startup
};
const baseoptions = {
    'enable_pass_lang': {
        declare: '不翻译中文(简体)', default_value: true, change_func: self => {
            if (self.checked) sessionStorage.clear()
        }
    }, 'enable_pass_lang_cht': {
        declare: '不翻译中文(繁体)', default_value: true, change_func: self => {
            if (self.checked) sessionStorage.clear()
        }
    }, 'remove_url': {
        declare: '自动过滤url', default_value: true,
    }, 'show_info': {
        declare: '显示翻译源', default_value: true, option_enable: true
    }, 'fullscrenn_hidden': {
        declare: '全屏时不显示', default_value: true,
    }, 'replace_translate': {
        declare: '替换式翻译', default_value: false, option_enable: true
    }, 'compress_storage': {
        declare: '压缩缓存', default_value: false,
    }
};
...
const rules = [{
    name: '推特通用', matcher: /https:\/\/([a-zA-Z.]*?\.|)twitter\.com/, options: [{
        name: "推文",
    ...
    name: 'x通用', matcher: /https:\/\/([a-zA-Z.]*?.|)x\.com/, options: [{
        name: "推文",
        selector: baseSelector('div[dir="auto"][lang]'),
        textGetter: baseTextGetter,
        textSetter: options => {
    ...
    name: 'youtube pc通用', matcher: /https:\/\/www.youtube.com\/(watch|shorts|results\?)/, options: [{
        name: "评论区",
    ...
    name: 'youtube mobile通用', matcher: /https:\/\/m.youtube.com\/watch/, options: [{
        name: "评论区",
    ...
    name: 'youtube 短视频', matcher: /https:\/\/(www|m).youtube.com\/shorts/, options: [{
        name: "评论区",
    ...
    name: 'youtube 社区', matcher: /https:\/\/(www|m).youtube.com\/(.*?\/community|post)/, options: [{
        name: "评论区",
    ...
    name: 'facebook通用', matcher: /https:\/\/www.facebook.com\/.+/, options: [{
        name: "帖子内容",
    ...
    name: 'reddit通用', matcher: /https:\/\/www.reddit.com\/.*/, options: [{
        name: '帖子标题',
    ...
    name: '5ch评论', matcher: /http(|s):\/\/(.*?\.|)5ch.net\/.*/, options: [{
        name: "标题",
    ...
    name: 'discord聊天', matcher: /https:\/\/discord.com\/.+/, options: [{
        name: "聊天内容",
    ...
    name: 'telegram聊天新', matcher: /https:\/\/.*?.telegram.org\/(a|z)\//, options: [{
        name: "聊天内容",
    ...
    name: 'telegram聊天', matcher: /https:\/\/.*?.telegram.org\/.+/, options: [{
        name: "聊天内容",
    ...
    name: 'quora通用', matcher: /https:\/\/www.quora.com/, options: [{
        name: "标题",
    ...
    name: 'tiktok评论', matcher: /https:\/\/www.tiktok.com/, options: [{
        name: "评论区",
    ...
    name: 'instagram评论', matcher: /https:\/\/www.instagram.com/, options: [{
        name: "评论区",
    ...
    name: 'threads', matcher: /https:\/\/www.threads.net/, options: [{
        name: "帖子",
    ...
    name: 'github', matcher: /https:\/\/github.com\/.+\/.+\/\w+\/\d+/, options: [{
        name: "issues",
    ...
    name: 'bsky', matcher: /https:\/\/bsky.app/, options: [{
        name: "主页帖子",
...
(function () {
    ...
    let main = _ => {
        ...
        for (const option of rule.options) {
            ...
            for (let i = 0; i < temp.length; i++) {
                ...
                const setterParams = {
                    element: now, translatorName: choice, rawText: rawText, rule: rule, option: option
                }
    ...
    PromiseRetryWrap(startup[GM_getValue('translate_choice', '谷歌翻译')]).then(() => {
...
function baseTextSetter({element, translatorName, text, rawText, rule, option}) {//change element text
    ...
    if (currentReplaceTranslate) {
        ...
        spanNode.style.cssText = "white-space:pre-wrap;display:block;margin:4px 0;padding:10px 14px;border-radius:8px;background:" + cardBg + ";border-top:" + cardBorder + ";font-size:inherit;line-height:1.6;color:inherit;";
        if (currentShowInfo) {
            const badge = document.createElement('span');
            badge.style.cssText = "display:flex;justify-content:center;align-items:center;gap:4px;font-size:11px;color:" + badgeColor + ";font-weight:500;margin-bottom:6px;padding:2px 8px;border-radius:10px;background:" + badgeBg + ";line-height:1;width:fit-content;margin-left:auto;margin-right:auto;";
        ...
        wrapper.style.cssText = "margin-top:8px;padding:10px 14px;border-radius:8px;background:" + cardBg + ";border-top:" + cardBorder + ";font-size:inherit;line-height:1.6;color:inherit;";
...
async function translate_ggm(raw) {
    const options = {
        method: "GET", url: "https://translate.google.com/m?tl=zh-CN&q=" + encodeURIComponent(raw), headers: {
            "Host": "translate.google.com",
        }, anonymous: true, nocache: true,
    }
...
function A(t, o, n) {
    ...
    if (!t) return null;
    const a = CryptoJS.enc.Hex.parse(m(o)), r = CryptoJS.enc.Hex.parse(m(n)), i = CryptoJS.AES.decrypt(t, a, {
        iv: r
    });
...
async function translate_tencent_startup() {
    ...
    const base_options = {
        method: 'GET', url: 'http://fanyi.qq.com', anonymous: true, headers: {
            "User-Agent": "test",
        }
    }
    ...
    const options = {
        method: 'POST', url: 'https://fanyi.qq.com/api/' + uri
    }
    const res = await Request(options);
...
async function translate_caiyun_startup() {
    ...
    const options = {
        method: "POST", url: 'https://api.interpreter.caiyunai.com/v1/user/jwt/generate', headers: {
            "Content-Type": "application/json",
...
async function translate_caiyun(raw) {
    ...
    const dic = [..."ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"].reduce((dic, current, index) => {
        dic[current] = source[index];
        return dic
    }, {});
    const decoder = line => Base64.decode([...line].map(i => dic[i] || i).join(""))
    const options = {
        method: "POST", url: 'https://api.interpreter.caiyunai.com/v1/translator', data: JSON.stringify({
            "source": raw.split('\n'),
            "trans_type": "auto2zh",
            "detect": true,
...
async function translate_papago_startup() {
    ...
    const base_options = {
        method: 'GET', url: 'https://papago.naver.com/', anonymous: true,
    }
    const base_res = await Request(base_options)
    const uri = /"\/(vendors~home\..*?.chunk.js)"/.exec(base_res.responseText)[1]
    const options = {
        method: 'GET', url: 'https://papago.naver.com/' + uri
    }
    const res = await Request(options);
...
async function translate_deepl(raw) {
    ...
    const data = {
        jsonrpc: '2.0', method: 'LMT_handle_texts', id, params: {
            splitting: 'newlines', lang: {
                source_lang_user_selected: 'auto', target_lang: 'ZH',
            }, texts: [{
                text: raw, requestAlternatives: 3
    ...
    const options = {
        method: 'POST', url: 'https://www2.deepl.com/jsonrpc', data: postData, headers: {
            'Content-Type': 'application/json',
            'Host': 'www.deepl.com',
...
async function translate_tencentai(raw) {
    const data = {
        "header": {
            ...
            "session": "",
            "user": ""
        }, "type": "plain", "model_category": "normal", "text_domain": "", "source": {
            "lang": "auto", "text_list": [raw]
        }, "target": {
            "lang": "zh"
        }
    }
    const options = {
        method: 'POST', url: 'https://transmart.qq.com/api/imt', data: JSON.stringify(data), headers: {
            'Content-Type': 'application/json',
...
async function PromiseRetryWrap(task, options, ...values) {
    ...
    const usedErrProcesser = ErrProcesser || (err => {
        throw err
    });
    if (!task) return;
    while (true) {
